...
async function fetchProductDetails() {
    try {
        ...
        try {
            ...
            imagesArray = data.image_url ? [data.image_url] : [];
        }
        ...
        if (thumbContainer && imagesArray.length > 1) {
            thumbContainer.innerHTML = imagesArray.map((img, index) => `
                <img src="${img}" 
                     ...
                     id="thumb-${index}"
                     class="w-20 h-20 md:w-24 md:h-24 rounded-xl object-cover cursor-pointer border-2 ${index === 0 ? 'border-pink-500' : 'border-transparent'} hover:border-pink-400 transition-all flex-none shadow-sm snap-center bg-gray-50">
            `).join('');
        }
        ...
        if(document.getElementById('decor-rating-stars')) {
        ...
        if (pkgList) {
            ...
            if (includes && includes.length > 0) {
                pkgList.innerHTML = includes.map(item => `<li class="flex items-center gap-2 mb-2"><span class="text-pink-500 font-bold">✓</span> ${item}</li>`).join('');
            } else {
        ...
        if(document.getElementById('loading-state')) {
            document.getElementById('loading-state').textContent = "Failed to load product details. Please refresh the page.";
        }
    }
}
